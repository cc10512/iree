...
#include "iree/hal/drivers/mpi/mpi_device.h"
...
//===----------------------------------------------------------------------===//
// iree_hal_mpi_device_t
//===----------------------------------------------------------------------===//

typedef struct iree_hal_mpi_device_t {
  ...
  iree_hal_mpi_device_params_t params;
} iree_hal_mpi_device_t;

static const iree_hal_device_vtable_t iree_hal_mpi_device_vtable;

static iree_hal_mpi_device_t* iree_hal_mpi_device_cast(
    iree_hal_device_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_mpi_device_vtable);
  return (iree_hal_mpi_device_t*)base_value;
}

void iree_hal_mpi_device_params_initialize(
    iree_hal_mpi_device_params_t* out_params) {
...
static iree_status_t iree_hal_mpi_device_check_params(
...
static iree_status_t iree_hal_mpi_device_create_internal(
    iree_hal_driver_t* driver, iree_string_view_t identifier,
    const iree_hal_mpi_device_params_t* params, CUdevice cu_device,
    CUstream stream, CUcontext context, iree_hal_mpi_dynamic_symbols_t* syms,
    iree_allocator_t host_allocator, iree_hal_device_t** out_device) {
  iree_hal_mpi_device_t* device = NULL;
  iree_host_size_t total_size = iree_sizeof_struct(*device) + identifier.size;
  IREE_RETURN_IF_ERROR(
      iree_allocator_malloc(host_allocator, total_size, (void**)&device));
  memset(device, 0, total_size);
  iree_hal_resource_initialize(&iree_hal_mpi_device_vtable, &device->resource);
  ...
  if (iree_status_is_ok(status) &&
    ...
    status = iree_hal_mpi_stream_command_buffer_create(
...
iree_status_t iree_hal_mpi_device_create(
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(z0,
                                    iree_hal_mpi_device_check_params(params));
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hal_mpi_device_create_internal(driver, identifier, params,
...
static void iree_hal_mpi_device_destroy(iree_hal_device_t* base_device) {
  iree_hal_mpi_device_t* device = iree_hal_mpi_device_cast(base_device);
  ...
  iree_hal_mpi_tracing_context_free(device->tracing_context);
...
static iree_string_view_t iree_hal_mpi_device_id(
    iree_hal_device_t* base_device) {
  iree_hal_mpi_device_t* device = iree_hal_mpi_device_cast(base_device);
  return device->identifier;
}

static iree_allocator_t iree_hal_mpi_device_host_allocator(
    iree_hal_device_t* base_device) {
  iree_hal_mpi_device_t* device = iree_hal_mpi_device_cast(base_device);
  return device->context_wrapper.host_allocator;
}

static iree_hal_allocator_t* iree_hal_mpi_device_allocator(
    iree_hal_device_t* base_device) {
  iree_hal_mpi_device_t* device = iree_hal_mpi_device_cast(base_device);
  return device->device_allocator;
}

static void iree_hal_mpi_replace_device_allocator(
...
static iree_status_t iree_hal_mpi_device_trim(iree_hal_device_t* base_device) {
  iree_hal_mpi_device_t* device = iree_hal_mpi_device_cast(base_device);
  iree_arena_block_pool_trim(&device->block_pool);
  return iree_hal_allocator_trim(device->device_allocator);
}

static iree_status_t iree_hal_mpi_device_query_i64(
  ...
  *out_value = 0;
  ...
  if (iree_string_view_equal(category,
    ...
    *out_value =
        iree_string_view_equal(key, iree_make_cstring_view("mpi-nvptx-fb"))
            ? 1
            : 0;
    return iree_ok_status();
  }
...
static bool iree_hal_mpi_nccl_id_is_empty(const iree_hal_mpi_nccl_id_t* id) {
...
static iree_status_t iree_hal_mpi_device_create_channel(
  ...
  if (iree_hal_mpi_nccl_id_is_empty(&id)) {
  ...
  return iree_hal_mpi_nccl_channel_create(&device->context_wrapper, &id, rank,
                                           count, out_channel);
}

static iree_status_t iree_hal_mpi_device_create_command_buffer(
  ...
  if (device->params.allow_inline_execution &&
    ...
    return iree_hal_mpi_stream_command_buffer_create(
  ...
  switch (device->params.command_buffer_mode) {
    case IREE_HAL_CUDA_COMMAND_BUFFER_MODE_GRAPH:
      return iree_hal_mpi_graph_command_buffer_create(
...
static iree_status_t iree_hal_mpi_device_create_descriptor_set_layout(
    iree_hal_device_t* base_device,
  ...
  iree_hal_mpi_device_t* device = iree_hal_mpi_device_cast(base_device);
  return iree_hal_mpi_descriptor_set_layout_create(
      &device->context_wrapper, flags, binding_count, bindings,
      out_descriptor_set_layout);
}

static iree_status_t iree_hal_mpi_device_create_event(
    iree_hal_device_t* base_device, iree_hal_event_t** out_event) {
  iree_hal_mpi_device_t* device = iree_hal_mpi_device_cast(base_device);
  return iree_hal_mpi_event_create(&device->context_wrapper, out_event);
}

static iree_status_t iree_hal_mpi_device_create_executable_cache(
    iree_hal_device_t* base_device, iree_string_view_t identifier,
    iree_loop_t loop, iree_hal_executable_cache_t** out_executable_cache) {
  iree_hal_mpi_device_t* device = iree_hal_mpi_device_cast(base_device);
  return iree_hal_mpi_nop_executable_cache_create(
      &device->context_wrapper, identifier, out_executable_cache);
}

static iree_status_t iree_hal_mpi_device_create_pipeline_layout(
    iree_hal_device_t* base_device, iree_host_size_t push_constants,
    iree_host_size_t set_layout_count,
    iree_hal_descriptor_set_layout_t* const* set_layouts,
    iree_hal_pipeline_layout_t** out_pipeline_layout) {
  iree_hal_mpi_device_t* device = iree_hal_mpi_device_cast(base_device);
  return iree_hal_mpi_pipeline_layout_create(
      &device->context_wrapper, set_layout_count, set_layouts, push_constants,
      out_pipeline_layout);
}

static iree_status_t iree_hal_mpi_device_create_semaphore(
    iree_hal_device_t* base_device, uint64_t initial_value,
    iree_hal_semaphore_t** out_semaphore) {
  iree_hal_mpi_device_t* device = iree_hal_mpi_device_cast(base_device);
  return iree_hal_mpi_semaphore_create(&device->context_wrapper, initial_value,
                                        out_semaphore);
}

static iree_hal_semaphore_compatibility_t
iree_hal_mpi_device_query_semaphore_compatibility(
    iree_hal_device_t* base_device, iree_hal_semaphore_t* semaphore) {
  // TODO: implement CUDA semaphores.
  return IREE_HAL_SEMAPHORE_COMPATIBILITY_HOST_ONLY;
}

static iree_status_t iree_hal_mpi_device_queue_alloca(
    iree_hal_device_t* base_device, iree_hal_queue_affinity_t queue_affinity,
...
static iree_status_t iree_hal_mpi_device_queue_dealloca(
    iree_hal_device_t* base_device, iree_hal_queue_affinity_t queue_affinity,
    const iree_hal_semaphore_list_t wait_semaphore_list,
    const iree_hal_semaphore_list_t signal_semaphore_list,
...
static iree_status_t iree_hal_mpi_device_queue_execute(
    iree_hal_device_t* base_device, iree_hal_queue_affinity_t queue_affinity,
    const iree_hal_semaphore_list_t wait_semaphore_list,
    const iree_hal_semaphore_list_t signal_semaphore_list,
  ...
  for (iree_host_size_t i = 0; i < command_buffer_count; i++) {
    ...
    if (iree_hal_mpi_stream_command_buffer_isa(command_buffer)) {
    ...
    } else if (iree_hal_mpi_graph_command_buffer_isa(command_buffer)) {
      CUgraphExec exec =
          iree_hal_mpi_graph_command_buffer_handle(command_buffers[i]);
  ...
  CUDA_RETURN_IF_ERROR(device->context_wrapper.syms,
                       ...
                       "cuStreamSynchronize");
  iree_hal_mpi_tracing_context_collect(device->tracing_context);
  IREE_TRACE_ZONE_END(z0);

  return iree_ok_status();
}

static iree_status_t iree_hal_mpi_device_queue_flush(
    iree_hal_device_t* base_device, iree_hal_queue_affinity_t queue_affinity) {
  // Currently unused; we flush as submissions are made.
  return iree_ok_status();
}

static iree_status_t iree_hal_mpi_device_wait_semaphores(
...
// TODO: hook into the PMPI profiling interface.
// See Chapter 15 of the MPI 4.0 specification:
// https://github.com/mpi-forum/mpi-forum.github.io/blob/master/docs/mpi-4.0/mpi40-report.pdf
static iree_status_t iree_hal_mpi_device_profiling_begin(
...
static iree_status_t iree_hal_mpi_device_profiling_end(
    iree_hal_device_t* device) {
  // Unimplemented (and that's ok).
  return iree_ok_status();
}
...
static const iree_hal_device_vtable_t iree_hal_mpi_device_vtable = {
    .destroy = iree_hal_mpi_device_destroy,
    .id = iree_hal_mpi_device_id,
    .host_allocator = iree_hal_mpi_device_host_allocator,
    .device_allocator = iree_hal_mpi_device_allocator,
    .replace_device_allocator = iree_hal_mpi_replace_device_allocator,
    .trim = iree_hal_mpi_device_trim,
    .query_i64 = iree_hal_mpi_device_query_i64,
    .create_channel = iree_hal_mpi_device_create_channel,
    .create_command_buffer = iree_hal_mpi_device_create_command_buffer,
    .create_descriptor_set_layout =
        iree_hal_mpi_device_create_descriptor_set_layout,
    .create_event = iree_hal_mpi_device_create_event,
    .create_executable_cache = iree_hal_mpi_device_create_executable_cache,
    .create_pipeline_layout = iree_hal_mpi_device_create_pipeline_layout,
    .create_semaphore = iree_hal_mpi_device_create_semaphore,
    .query_semaphore_compatibility =
        iree_hal_mpi_device_query_semaphore_compatibility,
    .transfer_range = iree_hal_device_submit_transfer_range_and_wait,
    .queue_alloca = iree_hal_mpi_device_queue_alloca,
    .queue_dealloca = iree_hal_mpi_device_queue_dealloca,
    .queue_execute = iree_hal_mpi_device_queue_execute,
    .queue_flush = iree_hal_mpi_device_queue_flush,
    .wait_semaphores = iree_hal_mpi_device_wait_semaphores,
    .profiling_begin = iree_hal_mpi_device_profiling_begin,
    .profiling_end = iree_hal_mpi_device_profiling_end,
};
